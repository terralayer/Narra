...
BOOK_SIGNALS = ("unabridged", "abridged", "audiobook", "narrated by", "narrator", "m4b", "mp3")
...
def classify_release(text: str) -> Classification:
    ...
    if any(ext in value for ext in AUDIO_FORMATS):
        score += 65
        reasons.append("audio-format")
    if any(ext in value for ext in ARCHIVE_FORMATS):
        score += 10
